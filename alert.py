...
and posts them to Discord. Only fires for regions within HOPS adjacency
steps of an Ireland home region: the set is seeded off the immutable
region.code prefix, then expanded over each region's live `neighbors`
graph, so it self-updates as the map changes.
...
# Home country whose regions seed the watch set, keyed off the immutable
# region.code prefix so Ireland's home regions stay watched even after
# conquest by another country.
HOME_COUNTRY_CODE = "ie"

# How many adjacency hops out from the home regions to watch. The game's
# neighbor graph is sea-aware, so 2 hops from island Ireland reaches the UK,
# Iceland and Portugal as well as transatlantic coasts (Canada, US, Greenland).
HOPS = 2
...
def compute_monitored_region_ids(regions, home_code=HOME_COUNTRY_CODE, hops=HOPS):
    """
    The set of region IDs to alert on: every region within `hops` adjacency
    steps of any home region. Home regions are seeded off the immutable
    region.code prefix (so Ireland's regions stay in the set even after
    conquest), then we breadth-first walk the live `neighbors` graph. Because
    the graph is rebuilt from each run's snapshot, the set self-updates as the
    map changes. Neighbor IDs that aren't present in this snapshot are skipped.
    """
    seed = {
        rid for rid, region in regions.items()
        if isinstance(region, dict)
        and origin_country_code(region.get("code")) == home_code
    }
    ...
    for _ in range(hops):
        ...
        for rid in frontier:
            ...
            if not isinstance(region, dict):
                continue
            for n in (region.get("neighbors") or []):
                if isinstance(n, str) and n in regions and n not in seen:
                    nxt.add(n)
        seen |= nxt
        frontier = nxt
        if not frontier:
...
def collect_bunker_upgrades(current_state, monitored_ids):
    """
...
def is_monitored(event, monitored_ids):
    return event.get("region_id") in monitored_ids
...
def format_event_embed(event):
    ...
    # ── Region + current controller token (clickable region name) ──
    region_tok = f"[**{region_name}**]({url}){origin_tag}"
    if not curr_cc:
        ctrl_tok = "🏳️ Unknown"
    else:
        # mark "occ." when the holder isn't the core owner
        occ = " occ." if (origin_cc and curr_cc != origin_cc) else ""
        ctrl_tok = f"{flag(curr_cc)} {country_label(curr_cc)}{occ}"

    # ── Compact detail token: what specifically changed ──
    if kind == "came_online":
        change_tok = f"online · L{c_run}"
    elif kind == "went_offline":
        change_tok = f"offline · was L{p_run or p_built_lv or '?'}"
    elif kind == "level_changed":
        change_tok = f"L{p_run} → L{c_run}"
    elif kind == "built":
        change_tok = f"built · L{c_built_lv or '?'}"
    elif kind == "destroyed":
        change_tok = f"destroyed · was L{p_built_lv or '?'}"
    elif kind == "ownership_changed":
        # controller token is replaced by the flip itself
        ctrl_tok = f"{flag(prev_cc)} {country_label(prev_cc)} → {flag(curr_cc)} {country_label(curr_cc)}"
        change_tok = ""
    elif kind == "construction_started":
        change_tok = f"building → L{c_built_lv}" if c_built_lv else "building"
    elif kind == "battle_started":
        change_tok = "battle started"
    elif kind == "battle_ended":
        change_tok = "battle ended"
    elif kind == "bunker_activating":
        up   = event.get("curr_bunker_upgrade") or {}
        dt   = parse_iso(up.get("will_be_active_at"))
        lvl  = up.get("level")
        lvl_txt = f" · L{lvl}" if isinstance(lvl, int) else ""
        # Discord renders <t:unix:R> as a live relative countdown.
        if dt:
            change_tok = f"activates <t:{int(dt.timestamp())}:R>{lvl_txt}"
        else:
            change_tok = f"pending{lvl_txt}"
        ...
        if isinstance(val, (int, float)) and isinstance(mx, (int, float)):
            change_tok = f"resistance full {int(val)}/{int(mx)} · liberation available"
        else:
            change_tok = "resistance full · liberation available"
    else:
        change_tok = "changed"
...
def run_alert():
    ...
    monitored_ids = compute_monitored_region_ids(regions)
    log(f"monitoring {len(monitored_ids)} region(s) within {HOPS} hop(s) "
        f"of {country_label(HOME_COUNTRY_CODE)}")
    ...
    for rid, up in collect_bunker_upgrades(current, monitored_ids).items():
        if rid in current:
    ...
    if not previous:
        ...
        post_ops_message(
            WEBHOOK_URL,
            f"first run, snapshotting **{len(current)}** regions worldwide. "
            f"Alerting on transitions within **{HOPS}** hop(s) of "
            f"{country_label(HOME_COUNTRY_CODE)} (**{len(monitored_ids)}** regions)."
        )
        save_state(current)
    ...
    events     = [e for e in all_events if is_monitored(e, monitored_ids)]
    log(f"detected {len(all_events)} transition(s), {len(events)} within {HOPS} hop(s) of {HOME_COUNTRY_CODE.upper()}")
...
def run_heartbeat():
    ...
    message = (
        ...
        f"Snapshotting **{regions_seen}** regions worldwide; alerting within "
        f"**{HOPS}** hop(s) of {country_label(HOME_COUNTRY_CODE)}."
    )
